import torch
import torch.nn as nn
import torch.nn.functional as functional

from depth_estimation.model.decoder_A_s import Decoder_A_Simple
from depth_estimation.model.decoder_recover import Decoder_recover
from .encoder_decoder import Encoder, Decoder
from .mViT import mViT
from .UNet import UNet

class SimpleEncoderDecoder(nn.Module):
    def __init__(self, debug=False) -> None:
        super(SimpleEncoderDecoder, self).__init__()

        self.debug = debug

        self.encoder = Encoder()
        self.decoder = Decoder()

    def forward(self, x):
        encoded_features = self.encoder(x)
        if self.debug:
            for feature in encoded_features:
                print(f"Encoded features shape: {feature.shape}")

        decoder_out = self.decoder(encoded_features)
        if self.debug:
            print(f"decoded features shape: {decoder_out.shape}")

        return decoder_out


class UDFNet(nn.Module):
    """Underwater Depth Fusion Net"""

    def __init__(self, n_bins=100, max_depth=None) -> None:
        super(UDFNet, self).__init__()

        # encoder based on MobileNetV2
        self.encoder = Encoder()

        # decoder
        self.decoder = Decoder(
            in_channels=1280,
            out_channels=3,

        )  # output N x 3 x 480 x 640

        self.decoder_recover=Decoder_recover(in_channels=1280,
            out_channels=3)

        self.decoder_A = Decoder_A_Simple(in_channels=1280,
                                   out_channels=3)

        # mViT
        self.mViT = mViT(
            in_channels=3,  # decoder output plus prior parametrization
            embedding_dim=48,
            patch_size=16,
            num_heads=4,
            num_query_kernels=48,
            n_bins=n_bins,
        )

        self.UNet = UNet(in_channels=1, out_channels=1)


        # regression for bin scores
        self.conv_out = nn.Sequential(
            nn.Conv2d(48, n_bins, kernel_size=1, stride=1, padding=0),
            nn.Softmax(dim=1),
        )


        # params
        self.n_bins = n_bins
        self.max_depth = max_depth


    def forward(self, I0 ,I45 ,I90 ,I135):
        """Input:
        - rgb: RGB input image, Nx3x480x640
        """
        # 1. 总光强
        I = (I0 + I45 + I90 + I135) * 0.5
        # 2. 水平-垂直、对角差分
        Q = I0 - I90
        U = I45 - I135
        # 3. 偏振度（DoP） 0~1
        eps = 1e-7
        dop = torch.clamp(torch.sqrt(Q * Q + U * U) / (I + eps), 0, 1)

        pol = torch.cat([I0, I45, I90, I135], dim=1)
        # encode
        encoder_out = self.encoder(pol)
        # decode
        ambient_out = self.decoder_A(encoder_out)
        t_out = self.decoder(encoder_out)
        recover_out=self.decoder_recover(encoder_out)


        mvit_in =  t_out

        # normed bin widths, range attention maps
        max_depth,bin_widths_normed, range_attention_maps = self.mViT(mvit_in)


        bin_edges_normed = torch.cumsum(bin_widths_normed, dim=1)
        bin_edges_normed = functional.pad(
            bin_edges_normed, (1, 0), value=0.0
        )  # add edge at zero

        # scale bin edges
        if self.max_depth is None:
            bin_edges = bin_edges_normed * max_depth
        else:
            bin_edges = bin_edges_normed * self.max_depth

        # bin centers
        bin_centers = 0.5 * (bin_edges[:, :-1] + bin_edges[:, 1:])

        # depth classification scores
        depth_scores = self.conv_out(range_attention_maps)

        # linear combination of centers and scores
        prediction = torch.sum(
            depth_scores * bin_centers.view(bin_centers.size(0), self.n_bins, 1, 1),
            dim=1,
            keepdim=True,
        )










        # print(f"I: {I.min():.3f} ~ {I.max():.3f}")
        # print(f"t: {t_out.min():.3f} ~ {t_out.max():.3f}")
        # print(f"A: {ambient_out.min():.3f} ~ {ambient_out.max():.3f}")
        # print(f"J: {J_out.min():.3f} ~ {J_out.max():.3f}")

        return prediction, bin_edges,recover_out,ambient_out,0





def test_udfnet():

    print("Testing UDFNet with random input ...")

    # instantiate model
    udfnet = UDFNet(n_bins=100)

    # generate random input
    random_rgb = torch.rand(4, 3, 480, 640)
    random_prior = torch.rand(4, 2, 240, 320)

    # inference
    out = udfnet(random_rgb, random_prior)

    print("Ok")


# to run this, use "python -m depth_estimation.model.model"
# otherwise the imports do not work as intended
# check https://stackoverflow.com/questions/72852/how-can-i-do-relative-imports-in-python
if __name__ == "__main__":
    # test_simple()
    test_udfnet()
