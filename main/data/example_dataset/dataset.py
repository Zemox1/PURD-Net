...
def get_example_dataset(train=False, shuffle=False, device="cpu"):
    ...
    rgb_depth_tuples = [i for i in lines]

    # transforms
    if train:
        input_transform = transforms.Compose(
            [
                IntPILToTensor(type="uint8"),  # load uint8 img as tensor
                transforms.ColorJitter(brightness=0.1, hue=0.05),  # color jitter
            ]
        )
        target_transform = transforms.Compose(
            [
                IntPILToTensor(type="uint8"),  # load float img as pytorch tensor
    ...
    # if not train
        ...
        target_transform = transforms.Compose(
            [
                FloatPILToTensor(),  # load float img as pytorch tensor
    ...
    dataset = InputTargetDataset(
        rgb_depth_tuples=rgb_depth_tuples,
...
def get_example_dataset_inference():


    index_file = "data/example_dataset/dataset.csv"
    lines = csv.reader(open(index_file).read().splitlines())
    rgb_target_tuples = [i for i in lines]

    # image transform
    image_transform = IntPILToTensor(type="uint8")


    dataset = InputDataset(rgb_target_tuples, image_transform)
